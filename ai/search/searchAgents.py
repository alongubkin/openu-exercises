  ...
  def getSuccessors(self, state):
    ...
    for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
      ...
      if not self.walls[nextx][nexty]:
        ...
        try:
          nextCorners[self.corners.index((nextx, nexty))] = 1
        except ValueError:
          # (nextx, nexty) is not a corner.
          pass
...
def cornersHeuristic(state, problem):
  ...
  # The first reference point is current pacman's position
  reference = state[0]

  # Unvisited corners
  corners = [problem.corners[corner_index] for corner_index, is_visited in enumerate(state[1]) 
    if not is_visited]

  # While there are still unvisited corners
  while len(corners) > 0:
    # Find the closest corner to the reference point
    current_min = 9999
    current_min_corner = None

    for corner in corners:
      # distance = util.manhattanDistance(reference, corner)
      distance = mazeDistance(reference, corner, problem.startingGameState)
      if current_min > distance:
        current_min = distance
        current_min_corner = corner

    # Add to the result
    result += current_min
    
    # Change reference point
    reference = current_min_corner
